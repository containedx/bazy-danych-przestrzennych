...
CREATE UNIQUE INDEX pracownicy_idx ON firma.pracownicy USING btree(id_pracownika);

COMMENT ON TABLE firma.pracownicy IS 'dane pracownikow';
COMMENT ON TABLE firma.godziny IS 'liczba godzin';
COMMENT ON TABLE firma.pensja_stanowisko IS 'stanowisko oraz zarobki';
COMMENT ON TABLE firma.premia IS 'przyznane premie';
COMMENT ON TABLE firma.wynagrodzenie IS 'wszystko';
...
INSERT INTO firma.godziny VALUES( 1, '2000-01-01', 100, 1, 8, 12);
...
SELECT imie, nazwisko FROM firma.pracownicy JOIN firma.wynagrodzenie ON firma.wynagrodzenie.id_pracownika=firma.pracownicy.id_pracownika JOIN firma.godziny ON firma.godziny.id_godziny=firma.wynagrodzenie.id_godziny JOIN firma.premia ON firma.premia.id_premii=firma.wynagrodzenie.id_premii WHERE liczba_godzin > 160 AND rodzaj LIKE 'brak';
...
DELETE FROM firma.pracownicy WHERE firma.pracownicy.id_pracownika IN (SELECT firma.pracownicy.id_pracownika FROM firma.pracownicy JOIN firma.wynagrodzenie ON firma.wynagrodzenie.id_pracownika = firma.pracownicy.id_pracownika JOIN firma.pensja_stanowisko ON firma.pensja_stanowisko.id_pensji = firma.wynagrodzenie.id_pensji WHERE kwota < 1200);

----------------zadanie 8
SELECT '+48' || telefon FROM firma.pracownicy;
SELECT LEFT(telefon, 3)||'-'||SUBSTRING(telefon, 4, 3)||'-'||RIGHT(telefon,3) FROM firma.pracownicy;
...
----------------zadanie 9 - RAPORT 
SELECT 'Pracownik '||imie||' '||nazwisko||' w dniu '||wynagrodzenie.data||' otrzymal pensje calkowita na kwote '||firma.pensja_stanowisko.kwota||' gdzie wynagrodzenie zasadnicze to '||firma.pensja_stanowisko.kwota-firma.premia.kwota||' premia: '||firma.premia.kwota||' nadgodziny: '||godziny.liczba_godzin-160 RAPORT FROM firma.pracownicy JOIN firma.wynagrodzenie ON firma.pracownicy.id_pracownika=firma.wynagrodzenie.id_pracownika  JOIN firma.pensja_stanowisko ON firma.pensja_stanowisko.id_pensji=firma.wynagrodzenie.id_pensji JOIN firma.premia ON firma.premia.id_premii = firma.wynagrodzenie.id_premii JOIN firma.godziny ON firma.wynagrodzenie.id_godziny=firma.godziny.id_godziny;
